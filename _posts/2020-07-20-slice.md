...
This question will investigate and implement Markov Chain Monte Carlo sampling technique slice sampling. Slice sampling allows us to draw samples from a target distribution, similar to the previous algorithms we have studied. In this method, a distribution is sampled by sampling uniformly from the region under the plot of its density function.
...
Slice sampling follows a set of basic steps:
    ...
    1. Choose a random starting value of x, let's call this $x_0$ where $f(x_0) > 0$
